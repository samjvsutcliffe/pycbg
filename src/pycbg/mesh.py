...
class Mesh():
    ...
    def __init__(self, dimensions, ncells, origin=(0.,0.,0.), directory="", check_duplicates=True, cell_type="ED3H8", round_decimal=None):
        ...
        elif cell_type=='ED2Q16G': self.nn_percell, self.n_dims = 4,2 #16, 2
...
def gen_gimp_mesh(length,cells,outdir="consol",mesh=None):
    ...
    verts = np.zeros((verts[0]*verts[1],2))
    for x in range(cells[0]+1):
        for y in range(cells[1]+1):
    ...
    if mesh:
        ...
        for n in verts:
            mesh.cells.append(n.astype(int))
    return elements
