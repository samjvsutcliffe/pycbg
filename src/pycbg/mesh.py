...
class Mesh():
    ...
    def __init__(self, dimensions, ncells, origin=(0.,0.,0.), directory="", check_duplicates=True, cell_type="ED3H8", round_decimal=None):
        ...
        if cell_type == 'ED2GIMP':
            gen_gimp_mesh(origin,self.dimensions,self.ncells,None,self)
        elif cell_type == 'ED3GIMP':
            gen_gimp_mesh_3d(origin,self.dimensions,self.ncells,None,self)
        else:
            self.write_file()
    ...
    def write_file(self, filename="mesh"):
        ...
        if self.cell_type == 'ED2GIMP':
            ...
            gen_gimp_mesh(self.origin,self.dimensions,self.ncells,self.directory,self)
        if self.cell_type == 'ED3GIMP':
            print("Creating GIMP mesh")
            gen_gimp_mesh_3d(self.origin,self.dimensions,self.ncells,self.directory,self)
...
gimp_order = [
        ...
        [2 ,2  ],
        [-1, 2 ]]
def gen_gimp_mesh(origin,length,cells,outdir=None,mesh=None):
    nd = 2
    ...
    for x in range(cellvs[0]):
        for y in range(cellvs[1]):
            verts[id_from_pos(x,y,cells),0] = origin[0] + (x*resolution[0])
            verts[id_from_pos(x,y,cells),1] = origin[1] + (y*resolution[1])
...
def in_bounds_3d(x,y,z,cells):
    return (x>=0) and (y>=0) and (z>=0) and (x<=cells[0]) and (y<=cells[1]) and (z<=cells[2])
def id_from_pos_3d(x,y,z,cells):
    return x + (y*(cells[0]+1)) + (z*(cells[0]+1)*(cells[1]+1))
def id_from_pos_cell_3d(x,y,cells):
    return x + (y*(cells[0])) + (y*(cells[0])*(cells[1]))
gimp_order_3d = [
        [0, 0  ,0],
        [1 ,0  ,0],
        [1 ,1  ,0],
...
def gen_gimp_mesh_3d(origin,length,cells,outdir=None,mesh=None):
    nd = 3
    length = np.array(length)
    cells = np.array(cells,dtype=int)
    cellvs = cells+1
    resolution = length/(cells)
    verts = np.zeros((cellvs[0]*cellvs[1]*cellvs[2],3))
    for x in range(cellvs[0]):
        for y in range(cellvs[1]):
            for z in range(cellvs[1]):
                verts[id_from_pos(x,y,z,cells),0] = origin[0] + (x*resolution[0])
                verts[id_from_pos(x,y,z,cells),1] = origin[1] + (y*resolution[1])
                verts[id_from_pos(x,y,z,cells),2] = origin[2] + (z*resolution[2])
    elements = np.zeros(((cells[0])*(cells[1])*(cells[2]),4**3))
    for e_x in range(cells[0]):
        for e_y in range(cells[1]):
            for e_z in range(cells[2]):
                ni = 0
                ei = id_from_pos_cell(e_x,e_y,e_z,cells)
                for ni in range(4**3):
                    dx,dy,dz = gimp_order[ni]
                    if in_bounds(e_x+dx,e_y+dy,e_z+dz,cells):
                        elements[ei,ni] = id_from_pos(e_x+dx,e_y+dy,e_z+dz,cells)
                    else:
                        elements[ei,ni] = -1
    if outdir != None:
        with open("{}/mesh.txt".format(outdir),"w") as f:
    ...
    if mesh:
        mesh.cells = []
        for e in elements:
            mesh.cells.append(e.astype(int).tolist()[0:8])
        mesh.nodes = verts
    return elements
